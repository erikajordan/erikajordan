...
<body>
    ...
    if (isset($_POST["add-joborder-submit"])) {
        ...
        $sql = "INSERT INTO joborder (Name, Date, Address, Number, Device_Type, Accessories, Model_Number, Brand_Name, Serial_Number, Complain, Warranty, Type_of_Repair, Status) VALUES ('$add1', '$add2', '$add3', '$add4', '$add5', '$add6', '$add7', '$add8', '$add9', '$add10', '$add11', '$add12' , 'Pending')";
    ...
    if (isset($_POST["add-joborder-final-submit"])) {
        ...
        $add10 = $_POST["type"];
        $add11 = $_POST["name"];
        $add12 = $_POST["device_type"];
        $add13 = $_POST["brand_name"];
        ...
        $conn->query("INSERT INTO schedule(Name, Device, Details, Type, Date) VALUES ('$add11', '$add12', '$add13', '$add10', '$add9')");
    }
    ...
    if (isset($_POST["add-installation-submit"])) {
        ...
        $sql = "INSERT INTO installation (Name, Date, Address, Number, Brand_Name, Model_Name, Model_Number, Serial_Number, Labor, Total, Status) VALUES ('$add1','$add2','$add3','$add4','$add5','$add6','$add7','$add8','$add9','$add10','Pending')";

        $conn->query($sql);

        $jon = $conn->query("SELECT Order_Number FROM installation WHERE Name = '$add1'")->fetch_object()->Order_Number;


        // $conn->query("INSERT INTO schedules VALUES ('$jon', '$add1', 'Installation', '$add2')");

        $conn->query("INSERT INTO schedule(Name, Device, Details, Type, Date) VALUES ('$add1', 'Aircon', '$add5', 'Installation', '$add2')");

        // DEDUCT item FROM Inventory
          $quantity = $conn->query("SELECT Quantity FROM inventory WHERE Name='$add5'")->fetch_object()->Quantity;
          $subtracted = $quantity - 1;
          $conn->query("UPDATE inventory SET Quantity = '$subtracted' WHERE Name='$add5'");
    }
    ...
    if (isset($_POST["update-installation-submit"])) {
        ...
        $add11 = $_POST['up-Status'];
        $add_id = $_POST['order-id'];

        $sql = "UPDATE installation SET Name = '$add1', Date = '$add2', Address = '$add3', Number = '$add4', Brand_Name = '$add5', Model_Name = '$add6',Model_Number = '$add7', Serial_Number = '$add8', Labor = '$add9', Total = '$add10', Status = '$add11' WHERE Order_Number = '$add_id'";

        $conn->query($sql);
    }

    // Schedule Add Form
    if (isset($_POST["add-schedule-submit"])) {
        $add1 = $_POST['sched_name'];
        $add2 = $_POST['sched_device'];
        $add3 = $_POST['sched_details'];
        $add4 = $_POST['sched_type'];
        $add5 = $_POST['sched_date'];

        $conn->query("INSERT INTO schedule(Name, Device, Details, Type, Date) VALUES ('$add1', '$add2', '$add3', '$add4', '$add5')");
    }
    ?>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if (intval($arr[0]) == $year) {
                            if (intval($arr[1]) == $month) {
                                if (intval($arr[2]) == $date)
                                    echo '<tr onclick = "editInputs(this, ' . "'schedules'" . ')"><td style="display: none;">' . $row['ION'] . '</td><td>' .  $row["Name"] . '</td><td style="display: none;">' . $row["Device"] . '</td><td style="display: none;">' . $row["Details"] . '</td><td>' . $row["Type"] . '</td><td>' . $row["Date"] . '</td></tr>';
                            }
                        }
                        $i++;
                    }
    ...
    <div class="wrapper">
        ...
        <div class="back-filter" id="forms" style="display: none;">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" style="display: none;">
                ...
                <input type="text" id="add-joborder7" value="" name="model_number">
                <input type="text" id="add-joborder8" value="" name="brand_name">
                <input type="text" id="add-joborder9" value="" name="serial_number">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" style="display: none;">
                ...
                <input type="text" id="update-installation11" name="up-Status">
                <input type="number" id="update-installation12" name="order-id">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" style="display: none;">
                ...
                <input type="text" id="add-schedule1" name="sched_name">
                <input type="text" id="add-schedule2" name="sched_device">
                <input type="text" id="add-schedule3" name="sched_details">
                <input type="text" id="add-schedule4" name="sched_type">
                <input type="text" id="add-schedule5" name="sched_date">

                <button type="submit" id="add-schedule-submit" name="add-schedule-submit"></button>
            ...
            <div class="add-joborder add-joborder-final" id="add-joborder-final" style="display: none;">
                ...
                <div class="input-section">
                    ...
                    <section>
                        <!-- Model Number -->
                        <i class="fas fa-arrow-circle-right"></i>
                        <input type="text" placeholder="Model Number" id="final-input7" class="final-inputfield" readonly>
                        <!-- Brand Name -->
                        <i class="fas fa-arrow-circle-right"></i>
                    ...
                    <section>
                        <!-- Serial Number -->
                        <i class="fas fa-arrow-circle-right"></i>
                        <input type="text" placeholder="Serial Number" id="final-input9" class="final-inputfield" readonly>
                        <!-- Complain -->
                        <i class="fas fa-arrow-circle-right"></i>
                    ...
                    <!-- Button for Duplicate and Remove Duplicate -->
            ...
            <div class="add-joborder" id="add-joborder" style="display: none;">
                ...
                <div class="input-section">
                    ...
                    <section>
                        ...
                        <input type="text" placeholder="Model Number" id="input7" class="inputfield">
                        <!-- Brand Name -->
                        <i class="fas fa-arrow-circle-right"></i>
                    ...
                    <section>
                        ...
                        <input type="text" placeholder="Serial Number" id="input9" class="inputfield">
                        <!-- Complain -->
                        <i class="fas fa-arrow-circle-right"></i>
            ...
            <div class="add-employee" id="add-employee" style="display: none;">
                ...
                <div class="input-section">
                    ...
                    <section>
                        ...
                        <input type="number" placeholder="TIN" class="employee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="employee-error3" onclick="popoff(this)">This field is requred</span>
                        <input type="number" placeholder="SSS" class="employee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="employee-error4" onclick="popoff(this)">This field is requred</span>
                        <input type="number" placeholder="PAGIBIG" class="employee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
            ...
            <div class="add-employee update-employee" id="update-employee" style="display: none;">
                ...
                <div class="input-section">
                    ...
                    <section>
                        ...
                        <input type="number" placeholder="TIN" class="upemployee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="upemployee-error3" onclick="popoff(this)">This field is requred</span>
                        <input type="number" placeholder="SSS" class="upemployee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="upemployee-error4" onclick="popoff(this)">This field is requred</span>
                        <input type="number" placeholder="PAGIBIG" class="upemployee-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
            ...
            <div class="add-installation add-joborder" id="add-installation" style="display: none;">
                ...
                <div class="heading">
                    <div class="text">
                        ...
                        <p>Vinzons Ave, Brgy V, Daet, Camarines Norte <br> 054-885-0659 /054-885-0632 <br> 0995-511-6300 / 0999-734-6666</p>
                        <p>Fill in Information Needed</p>
                    </div>
                    <img src="resources/logo.png" alt="">
                </div>
            ...
            <div class="add-installation add-joborder" id="update-installation" style="display: none;">
                ...
                <img src="resources/document-download-icon.png" alt="" class="download-button" onclick="downloadInstallation()" id='installation-download'>
                <div id="installation-print">
                <div class="heading" id="reciept-heading">
                        <img src="resources/logo.png" alt="">
                        <div class="text">
                            <h4>Roll Electornics</h4>
                    ...
                    <div class="heading" id="reciept-heading2">
                        ...
                        <div id="reciept-jon">
                            No:.
                            <div class="reciept-input" id="installation-no" style="border: none;
                ...
                color: rgba(0,0,0,0.6);">
                    <section>
                        ...
                        <input type="text" placeholder="Name" class="update-installation-input" style="
                        border: none;
                    ...
                    <div class="bottom" style='text-align: left;
                        ...
                        <div class='input-field' style='width: 40%;display: grid;
                        grid-template-columns: 1fr 1fr;
                        align-items: center;
                        justify-content: center;
                        box-sizing: border-box;'>
                    ...
                    <input type='hidden' class='update-installation-input' id="installation_status">

                    <span style='display:none;'>No. <input id='update-installation-no' type='number' class='update-installation-input' readonly /></span>



                    <!-- Status Toggle -->
                    <label class="toggle" id='finish-toggle'>
                        <input type="checkbox" id="toggleSwitch" onclick="toggleSwitch('toggleSwitch', 'toggle_status')">
                        <span class="slider"></span>
                    </label>
                    <p id="toggle_status"></p>

                    <button class="submit" id='update-installation-button' onclick="checker('forms','update-installation', 'update-installation-input','upinstallation-error')"
            ...
            <div class="add-inventory" id="add-schedule" style="display: none;">
                <div class="back-button" onclick="closer('forms','add-schedule')">
                    <img src="resources/back_icon.png" alt="">
                </div>
                <div class="heading">
                    <div class="text">
                        <h4>Schedule Add Form</h4>
                        <p>Fill in Information Needed</p>
                    </div>
                    <img src="resources/logo.png" alt="">
                </div>
                ...
                <div class="input-section">
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="schedule-error1" onclick="popoff(this)">This field is requred</span>
                        <input type="text" placeholder="Name" class="schedule-input">
                    </section>
                    <section>
                        <i class="far fa-address-card"></i>
                        <span class="error" id="schedule-error2" onclick="popoff(this)">This field is requred</span>
                        <input type="text" placeholder="Device" class="schedule-input">
                    </section>
                    <section>
                        <i class="fas fa-tag"></i>
                        <span class="error" id="schedule-error3" onclick="popoff(this)">This field is requred</span>
                        <input type="text" placeholder="Details" class="schedule-input">
                    </section>
                    <section>
                        <i class="fas fa-arrow-circle-right"></i>
                        <span class="error" id="schedule-error4" onclick="popoff(this)">This field is requred</span>
                        <input type="text" placeholder="Type" class="schedule-input">
                    </section>
                    <section>
                        <i class="fas fa-arrow-circle-right"></i>
                        <span class="error" id="schedule-error5" onclick="popoff(this)">This field is requred</span>
                        <input type="date" placeholder="Date" class="schedule-input">
                    </section>
                    <div>
                        <button class="submit" onclick="checker('forms','add-schedule', 'schedule-input', 'schedule-error')">Add</button>
                    </div>
                </div>
        ...
        <div class="sidebar">
            ...
            <div class="button-section">
                ...
                <div class="nav-button" id="nb4" onclick="schedulesButton()">Service Schedules</div>
        ...
        <div style="height: 101%; overflow: hidden;">
            <svg viewBox="0 0 500 150" preserveAspectRatio="none" style="height: 100%; width: 100%;">
                <path d="M395.47,-2.30 C-320.58,76.64 806.11,46.42 201.02,149.14 L-0.00,149.14 L-0.00,0.00 Z" style="stroke: none; fill: #3498DB;"></path>
            </svg>
        </div>
        ...
        <div class="content-panel">
            ...
            <div class="panel-joborder" id="panel-joborder">
                ...
                <div class="table-div">         
                    <div class="joborder-table">
                    <table  id="joborder-table">
                        <!-- PHP Codes for table -->
                        <?php
                        ...
                        if ($result->num_rows > 0) {
                            $i = 0;
                            while ($row = $result->fetch_assoc()) {
                                echo '<tr onclick = "editJobOrder(this)"><td>' . $row['JON'] . '</td><td>' .  $row["Name"] . '</td><td>' . $row["Date"] . '</td><td style="display: none;">' . $row["Address"] . '</td><td style="display: none;">' . $row["Number"] . '</td><td>' . $row["Device_Type"] . '</td><td style="display: none;">' . $row["Accessories"] . '</td><td style="display: none;">' . $row["Model_Number"] . '</td><td style="display: none;">' . $row["Brand_Name"] . '</td><td style="display: none;">' . $row["Serial_Number"] . '</td><td style="display: none;">' . $row["Complain"] . '</td><td>' . $row["Warranty"] . '</td><td style="display: none;">' . $row["Type_of_Repair"] . '</td><td style="display: none;">' . $row["Findings"] . '</td><td style="display: none;">' . $row["Materials"] . '</td><td style="display: none;">' . $row["Check_up"] . '</td><td style="display: none;">' . $row["Labor"] . '</td><td style="display: none;">' . $row["Total"] . '</td><td style="display: none;">' . $row["Technician"] . '</td><td style="display: none;">' . $row["Warranty_exp"] . '</td><td>' . $row["Status"] . '</td></tr>';
                                $i++;
                            }
                        }
                        ?>
            ...
            <div class="panel-inventory" id="panel-inventory" style="display: none">
                <div class="button-div">
                    ...
                    <table class="actual-table" id="logs-table1">
                        <tr class="table-heading">
                    ...
                    <table class="actual-table" id="logs-table2" style="display: none;">
                        <tr class="table-heading">
                            <th>Date</th>
                            <th>Product_Name</th>
                ...
                <div class="table-div">
                    ...
                    <!-- Inventory Logs -->
                    <div id="inventory-logs" class="inventory-logs" style="display: none; height: 100%;">
            ...
            <div class="panel-schedules" id="panel-schedules" style="display: none;">
                <div class="button-div">
                    <div class="add-button" onclick="popout('forms','add-schedule')"><i class="fas fa-plus-square fa-2x"></i>Add</div>
                    <input id="joborder-search" type="text" placeholder="Search Name..." onkeyup="search()">
                    <table class="actual-table">
                        <tr class="table-heading">
                            <th>Customer Name</th>
                ...
                <div class="table-div">
                    ...
                    <table id="schedules-table">                       
                        ...
                        if ($result->num_rows > 0) {
                            $i = 0;
                            while ($row = $result->fetch_assoc()) {
                                echo '<tr onclick = "editInputs(this, ' . "'schedules'" . ')"><td style="display: none;">' . $row['ION'] . '</td><td>' .  $row["Name"] . '</td><td>' . $row["Device"] . '</td><td>' . $row["Details"] . '</td><td>' . $row["Type"] . '</td><td>' . $row["Date"] . '</td><td>' . $row["Status"] . '</td>';
                                ...
                                if($row["Status"] == "Pending"){
                                    if (intval($arr[0]) == $year) {
                                        if (intval($arr[1]) == $month) {
                                            if (intval($arr[2]) <= $date){
                                                $aging = $date - intval($arr[2]);
                                                echo '<td>' . $aging . ' day(s)</td></tr>';
                                            }
                                        }
                                    }
                                }else{
                                    echo '<td>0 day(s)</td></tr>';
                                }
                                $i++;
                            }
            ...
            <div class="panel-installation" id="panel-installation" style="display: none;">
                <div class="button-div">
                    ...
                    <input id="joborder-search" type="text" placeholder="Search Name..." onkeyup="search()">
                    <div id="filter-button-div">
                        <button class="filter-buttons fbtns" onclick="filterInstallation(this,'')" style="background-color: #3498DB;">All</button>
                        <button class="filter-buttons fbtns" onclick="filterInstallation(this,'Pending')">Pending</button>
                        <button class="filter-buttons fbtns" onclick="filterInstallation(this,'Finished')">Finished</button>
                    </div>
                    <table class="actual-table">
                        <tr class="table-heading">
