...
function filterInstallation(that,str){
    // console.log('Filter Pending');
    var filter, table, tr, td, txtvalue, buttons; 
    filter = str
    table = document.getElementById('installation-table')
    tr = table.getElementsByTagName('tr')
...
function showLogs(that,id){
    switch(id){
        case 'inventory-table':
            ...
            document.getElementById('logs-table2').style.display = 'none'
            document.getElementById('logs-table1').style.display = ''
        break
        ...
        case 'inventory-logs':
            ...
            document.getElementById('logs-table2').style.display = ''
            document.getElementById('logs-table1').style.display = 'none'
        break
    }
}
...
function editJobOrder(that){
    ...
    if(td.length > 4 && td[20].innerText == 'Pending'){
        ...
        document.getElementById('add-joborder5').value = td[5].innerText  //to add to details
        document.getElementById('add-joborder8').value = td[8].innerText  //to add to details
...
function editInstallation(that){
  ...
  for(let i=1; i<inputFields.length; i++){
    ...
    document.querySelector('#installation-no').innerText = parseInt(td[0].innerText)

    var inst_stat = document.getElementById('installation_status')
    var inst_toggle = document.getElementById('toggleSwitch')

    if(inst_stat.value == 'Finished'){
        inst_toggle.checked = true
        document.querySelector('#installation-download').style.display = 'block'
    }
    else{
        inst_toggle.checked = false
        document.querySelector('#installation-download').style.display = 'none'
    }
    document.getElementById('toggle_status').innerText =  document.getElementById('installation_status').value
...
function downloadInstallation(){
  const submitBtn = document.querySelector('#update-installation-button')
  const toggleBtn = document.querySelector('#finish-toggle')
  ...
  toggleBtn.style.visibility = 'hidden'
    ...
    const element = document.getElementById('installation-print')
    html2pdf()
    .from(element)
    .save()
    console.log('success!');
...
function toggleSwitch(id1, id2){
    var toggle = document.getElementById(id1)
    var display = document.getElementById(id2)
    var status = document.getElementById('installation_status')
    
    if(toggle.checked == true){
        status.value = 'Finished'
        display.innerText = status.value
    }else{
